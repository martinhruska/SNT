...
Jelikož je RCPS problém NP-úplný \cite{artigues:2007}, tak se k~jeho řešení používají různé heuristické metody jako
...
Příklad vstupních dat lze najít v~adresáři \emph{input\_example}.
...
Jak bylo řečeno v~sekci \ref{sec:concept}, roste množství klausulí pokrytí (klausule \ref{eq:4}) exponenciálně,
...
Experimenty byly provedeny za použití SAT solveru MiniSAT pro sady j30 a j60 a SAT solveru
Glucose pro sadu j120, jelikož se tento SAT solver ukázal jako výkonnější, a tedy byl
použit pro vyhodnocení těchto náročnějších instancí.
Byl měřen čas procesu na CPU.
...
\scalebox{0.7}{
  \begin{tabular}{ | l | r | r | r | r | r | r | r | r | r |}
   \hline
    ...
    j120 & $30$ & $0.9$ & $2.8$ & $8.2$ & $6.9$ & $8.9$ & $5.3$ & $67$  \\ \hline
   \end{tabular}
}
...
\begin{figure}[tb]
    \begin{center}
    \includegraphics[scale=0.75]{dst_j120}
    \end{center}
    \caption{Doba výpočtu SAT solveru v~závislosti na vzdálenosti makespan od optima v~instanci RCPS problému redukovaného na SAT.
...
V~grafu jsou zaneseny jen výsledky, u~nichž byla doba řešení úlohy delší nebo rovna 1 sekundě.
Takovýchto případů bylo $80\%$ u~sady j30, $72\%$ u~sady j60 a $33\%$ u~sady j120.
...
Výsledky experimentů ovšem ukazují, že se podařilo řešit i obtížnější
instance RCPS problému, a tedy redukce RCPS problému na SAT problém
...
\\
  ...
  \begin{itemize}
    ...
    \begin{itemize}
        \item m $\ldots$ MiniSAT.
        \item g $\ldots$ Glucose (default).
    \end{itemize}
