...
/**
 * Transform instance of RCSP to precendece graph
 * @param[in] instance RCSP instance
 * @param[out] graph Graph to be filled
 */
int RCSPSolver::parseGraphFromRCSPInstance(PrecedenceGraph& graph,
...
/**
 * Store values from graph to RCSP instance.
 * It is supposed that values in graph was computed by
 * Floyd-warshall algorithm
 */
int RCSPSolver::storeGraphValuesToInstance(const PrecedenceGraph& graph,
        RCSPInstance& instance)
{
    ...
    for (int job=0; job <= lastJob-1; ++job)
    {
        ...
        instance.setEFinish(job, lStart+duration);
    }
